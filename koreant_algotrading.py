...
class Trader:
	...
	def compute_arb_conversion_order(self, state: TradingState, avg_running_price):
		product = 'ORCHIDS'
		observations = state.observations.conversionObservations[product]
		conversions = None
		...
		if state.position.get(product,0) > 0:
			if real_bid_price >= avg_running_price:
				conversions = -state.position[product]
		
		if state.position.get(product,0) < 0:
			if real_ask_price < avg_running_price:
				conversions = -state.position[product]
		print(f";real_bid;{real_bid_price};real_ask;{real_ask_price};avg_running_price;{avg_running_price};conversions;{conversions}")
		return conversions

	def compute_orders_orchids(self, state: TradingState, acc_bid, acc_ask, POSITION_LIMIT, conversions):
		product = 'ORCHIDS'
		...
		if (buy_cpos_update < pos_limt) and (cpos > 50): # long position
			num = min(200, pos_limt - buy_cpos_update)
			orders.append(Order(product, min(undercut_buy - 1, acc_bid-1), num))
			buy_cpos_update += num
		...
		# placing orders
		if (sell_cpos_update > -pos_limt) and (cpos > 0): # long position
			num = max(-200, -pos_limt-sell_cpos_update) # sell 40 or 
			orders.append(Order(product, max(undercut_sell-1, acc_ask+1), num))
			sell_cpos_update += num

		if (sell_cpos_update > -pos_limt) and (cpos < -50):
	...
	def run(self, state: TradingState):
		...
		print(f";own_trades;{state.own_trades.get(want_to_see_product,"")}")
		...
		#ORCHID conversion pnl update
		
		for product in state.own_trades.keys():
			for trade in state.own_trades[product]:
				if trade.timestamp != state.timestamp-100:
					continue
				mydata['volume_traded'][product] += abs(trade.quantity)
				if trade.buyer == "SUBMISSION":
					mydata['cpnl'][product] -= trade.quantity * trade.price
				else:
					mydata['cpnl'][product] += trade.quantity * trade.price
		"""
		...
		conversions = self.compute_arb_conversion_order(state, avg_running_price = average_price)

		nxt_OC_price = self.next_orchid_price(state)
		result['ORCHIDS'] = self.compute_orders_orchids(state, acc_bid = nxt_OC_price - 1, acc_ask = nxt_OC_price + 1, POSITION_LIMIT = mydata['position_limit'], conversions = conversions)
		#conversions = None
