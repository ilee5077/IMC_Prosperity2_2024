...
class Trader:
    ...
    def compute_orders_orchids_new(self, state:TradingState, mydata):
        ...
        print(f"buy_from_south:{buy_from_south},sell_to_south:{sell_to_south},midprice:{mid_price};{state.observations.conversionObservations[p].importTariff};{state.observations.conversionObservations[p].exportTariff};{state.observations.conversionObservations[p].transportFees}")
    ...
    def compute_orders_basket(self, order_depth, state: TradingState, POSITION_LIMIT, mydata):
        ...
        elif (res_cc < -trade_at_cc):# underprice buy
        ...
        '''
        if (res_cc > trade_at_cc): # overpriced, sell
            vol = state.position.get('GIFT_BASKET',0) + mydata['position_limit']['GIFT_BASKET']
            assert(vol >= 0)
            if vol > 0:
                orders['GIFT_BASKET'].append(Order('GIFT_BASKET', best_buy['GIFT_BASKET'], -vol))
        elif (res_cc < -trade_at_cc):# underprice buy
            vol = mydata['position_limit']['GIFT_BASKET'] - state.position.get('GIFT_BASKET',0)
            assert(vol >= 0)
            if vol > 0:
                orders['GIFT_BASKET'].append(Order('GIFT_BASKET', best_sell['GIFT_BASKET'], vol))

        elif res_cc < close_at_cc and state.position.get('GIFT_BASKET',0) < 0: 
            vol = -state.position.get('GIFT_BASKET',0)
            assert(vol >= 0)
            if vol > 0:
                orders['GIFT_BASKET'].append(Order('GIFT_BASKET', best_sell['GIFT_BASKET'], vol))
        elif res_cc > -close_at_cc and state.position.get('GIFT_BASKET',0) > 0:
...
'''
    ...
    def compute_orders_ccnc(self, state: TradingState, mydata):
        ...
        #res = np.log(mid_price['COCONUT']) -coef*np.log(mid_price['COCONUT_COUPON'])
        ...
        T = 250/365 # 115874/127837/154670/398381
        ...
        perc= 500
        std = 13.467
        sell_tot_order_for = int(round(sell_tot_order_for*perc,0))
        buy_cpos_ccn = state.position.get(p,0)
        if (buy_cpos_ccn < mydata['position_limit'][p]) and (abs(deviation) >= std) and (abs(deviation) < std*2): #coupon underpriced, buy now and sell later
            for ask, vol in sell_orders_ccn:
                if (sell_tot_order_for > 0) and (buy_cpos_ccn < mydata['position_limit'][p]):
                    order_for = min(mydata['position_limit'][p] - buy_cpos_ccn,-vol,sell_tot_order_for)
                    buy_cpos_ccn += order_for
                    sell_tot_order_for -= order_for
                    orders[p].append(Order(p,ask,order_for))
            
        
        buy_tot_order_for = int(round(buy_tot_order_for*perc,0))
        sell_cpos_ccn = state.position.get(p,0)
        if (sell_cpos_ccn > -mydata['position_limit'][p]) and (abs(deviation) >= std) and (abs(deviation) < std*2): #coupon overpriced, sell now and buy later
            for bid, vol in buy_orders_ccn:
                if (buy_tot_order_for > 0) and (sell_cpos_ccn > -mydata['position_limit'][p]):
                    order_for = min(mydata['position_limit'][p] + sell_cpos_ccn,vol,buy_tot_order_for)
                    sell_cpos_ccn += -order_for
                    buy_tot_order_for -= order_for
                    orders[p].append(Order(p,bid,-order_for))
        '''
